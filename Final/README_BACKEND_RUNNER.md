...
### **RL Step 6: Hyperparameter Sweep (Epsilon Decay)**
- **File:** `RL/decay_experiment.py`
- **Time:** 15-30 min (depends on number of decay values)
- **Does:** Runs multiple training sessions with different epsilon-decay values and plots average reward vs episodes
- **Command:** `python decay_experiment.py --decays 0.85 0.90 0.95 0.99 --episodes 200`
- **Output:** `decay_experiment_plot.png` and `decay_experiment_results.json`

### **RL Step 7: Generate Key & Encrypt with Trained Model**
- **File:** `RL/use_trained_model.py`
- **Time:** 10 sec (plus encryption)
- **Does:** Loads a saved DQN model, runs one episode to produce a final key,
  and optionally encrypts a user message using that key.
- **Command:** `python use_trained_model.py --model <path_to_model> --message "hello"`
- **Output:** Final key written to file, encrypted payload printed
